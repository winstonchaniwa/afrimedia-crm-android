...
@Composable
fun InvoicesListScreen(repository: Repository, onOpenInvoice: (Int) -> Unit, onNewInvoice: () -> Unit) {
    var invoices by remember { mutableStateOf<List<Document>>(emptyList()) }
    ...
    LaunchedEffect(refreshKey) {
        isLoading = true
        when (val result = repository.call { repository.api.listInvoices() }) {
            is ApiResult.Success -> { invoices = result.data; error = null }
    ...
    Scaffold(
        ...
        when {
            ...
            invoices.isEmpty() -> EmptyState("No invoices yet. Tap + to create one.", Modifier.padding(padding))
            else -> LazyColumn(
                modifier = Modifier.fillMaxSize().padding(padding),
                contentPadding = PaddingValues(bottom = 96.dp, top = 8.dp)
            ) {
                items(invoices, key = { it.id }) { invoice ->
                    DocumentRow(invoice, onClick = { onOpenInvoice(invoice.id) })
                    Divider()
                }
            }
        }
